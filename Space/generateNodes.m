function [nodes, h] = generateNodes(bands,info,powerWindow,locationWindow,nNodes,varargin) 
  ...
  nodes.center = [min(nodes.lat) + range(nodes.lat)/2, min(nodes.long) + range(nodes.long)/2];
  nodes.center = [nodes.center, interp2(nodes.long,nodes.lat,cbands.data,nodes.center(2), nodes.center(1))];
  nodes.powers = powerWindow(1) + rand(nNodes,1) * range(powerWindow);
