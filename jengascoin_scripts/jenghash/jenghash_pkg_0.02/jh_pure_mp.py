...
def build_hash_struct(out_size, seed, out_type='cache', coin='jng', thread_count=1):
    ...
    if out_type == 'cache':
        ...
        # with Parallel(n_jobs=thread_count, prefer="threads", require='sharedmem') as parallel:
        with Parallel(n_jobs=thread_count, prefer="processes") as parallel:
            hash_struct[i] = parallel(delayed(calc_dataset_item)(seed, i) for i in range(out_size))
            # inc_size = 1000
            # i = 0
            # while i < total_size:
                # n_iter = 0
                # while n_iter < (total_size//inc_size):
                    # hash_struct[i+n_iter] = parallel(delayed(calc_dataset_item)(cache, i for i in (range(4), 'abcde', 3))(cache, i+n_iter))
                        # # accumulator += sum(results)  # synchronization barrier
                    # n_iter += 1
                # i += inc_size
            # hashstruct = Parallel(n_jobs=thread_count, prefer="threads", require='sharedmem')(delayed(sqrt)(i**2) for i in range(10))
        # with SharedMemoryManager() as smm:
            # total_size = out_size // HASH_BYTES
            # # smm.start()  # Start the process that manages the shared memory blocks
            # shm = smm.SharedMemory(size=out_size)
            ...
            # for th in range(thread_count):
                # start.append((th * total_size) // thread_count)
                # stop.append(start[th] + (total_size // thread_count))
                # if th == (thread_count-1):
                    # stop[th] = total_size # make sure you get them all
            # print("starts: ", start)
            # print("stops:  ", stop)
            # for th in range(thread_count):
                # p.append(mp.Process(calc_dataset_inplace, args=(start[th], stop[th], cache, hash_struct, ))) 
                # p[th].start()
            # for th in range(thread_count):
                # p[th].join()
            # dataset = 
