...
class Verifier:  # high-level class
    ...
    def hashimoto_light(self, full_size, header, nonce, key=None):
        return hashimoto(header, nonce, full_size,
                         lambda x: self.smith.calc_dataset_item(x, key))
...
# ----- Full dataset calculation ----------------------------------------------
    def calc_dataset_chunk(self, i_start, chunk_len=1):
        ...
        for i in range(chunk_len):
            ...
            if isinstance(self.key, (bytes, bytearray)):
                mix = np.concatenate((mix, deserialize_hash(self.key)))
    ...
    def calc_dataset_item(self, i, key=None):
        n = len(self.cache)
        r = HASH_BYTES // WORD_BYTES
        i = int(i)
        # initialize the mix
        ...
        if isinstance(key, (bytes, bytearray)):
            mix = np.concatenate((mix, deserialize_hash(key)))
        elif isinstance(self.key, (bytes, bytearray)):
            mix = np.concatenate((mix, deserialize_hash(self.key)))
        mix = blake3_512(int_list_to_bytes(mix))
        # fnv it with a lot of random cache nodes based on i
        for j in range(DATASET_PARENTS):
    ...
    def calc_dataset(self):
        ...
        for i in range(total_size):
            dataset[i] = self.calc_dataset_item(i, self.key)
    ...
    def build_hash_struct(self, out_type, alt_seed=None, alt_size=None,
        ...
        if out_type == 'cache':
            ...
            if isinstance(self.key, (bytes, bytearray)):
                short_key = self.key.hex()[-4:]  # last four of the key
            else:
                short_key = ''
            partial_name = name_root + short_key + '_partial.npy'
...
def int_list_to_bytes(ls):
    return bytearray(struct.pack("{}I".format(len(ls)), *ls))
